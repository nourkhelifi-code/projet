...
// 🔒 Création d'événement (UNIQUEMENT pour utilisateurs authentifiés)
router.post("/create", requireLogin, (req, res, next) => {
    console.log("REQ.USER =", req.user); // Vérification si le user existe
    next();
}, createEvent);
...
router.get("/positions", getEventsPositions);
